...
I used AI (Claude) as a navigation and debugging aid on this project. Specific uses:

- **Tracing the notification call chain for Issue #4.** I asked AI to trace what happens
  when a friend rates a song, from the route down to the service. It followed
  `POST /songs/<id>/rate` → [routes/songs.py](routes/songs.py) `rate()` →
  `rate_song()` in [services/notification_service.py](services/notification_service.py),
  and pointed out that `rate_song` commits the rating and returns with no
  `create_notification` call — whereas `add_to_playlist`, in the same file, ends with a
  `create_notification(... "song_added_to_playlist" ...)` step. That side-by-side
  comparison is what localized the bug as a *missing* notification rather than a broken one.

- **Explaining the `playlist_entries` association table for Issue #5.** I asked AI why the
  last playlist song was missing and how playlist ordering worked. It explained that
  `playlist_entries` is a many-to-many table carrying `position`/`added_by`/`added_at`
  columns, that `get_playlist_songs` orders correctly by `asc(position)`, and that the
  drop happened in the return line `songs[:-1]` — a Python slice that discards the last
  element after the query had already fetched everything.

- **Drafting regression tests.** For features with no existing test (the "Friends
  Listening Now" feed and notifications), I had AI scaffold `pytest` cases that reproduce
  the bug first (e.g. a friend who listened 5 hours ago must *not* appear in
  [tests/test_feed.py](tests/test_feed.py)); I then ran them to confirm they failed before
  I touched the service code.

**Where I had to course-correct the AI (Issue #3 — "same song shows up twice in search").**
AI initially reported this as a live, reproducible bug caused by the `outerjoin(song_tags)`
in [services/search_service.py](services/search_service.py), removed the join, and wrote a
regression test it claimed would guard it. I was skeptical because the *existing* search
tests were all passing, so I reverted the change and re-ran `pytest tests/test_search.py` —
they still passed. Digging in, I verified the real behavior myself: SQLAlchemy 2.0's legacy
`db.session.query(Song).all()` auto-deduplicates entities by primary key, so the duplicate
rows the join produces are collapsed back to one before they ever reach the caller. That
also exposed that the AI's proposed regression test was worthless — it asserted on the
already-deduped output of `search_songs()`, so it passed with *or* without the join and
guarded nothing. Because the bug does not actually manifest in the running app, I decided to
drop Issue #3 and fix Issues #4 and #5 instead. This was a case where the AI's first
explanation was confidently wrong and I had to test the claim against the real code rather
than take it at face value.

All fixes and final decisions are my own. I used AI to trace call chains, explain unfamiliar
SQLAlchemy behavior, and scaffold tests — but I independently ran every test and verified the
key claims, and overrode the AI where its analysis didn't hold up.
...
**Services (`services/`)** — where all the business logic lives.
...
Notifications are generated by two friend actions on a shared song, both living in
`notification_service.py` and both following this same pattern: `add_to_playlist`
(adding the song to a playlist) and `rate_song` (rating the song). Each notifies the
song's original sharer via `create_notification`, skipping the case where the actor is the
sharer themselves.
...
- **Strict route/service/model layering.** Routes never touch the database directly;
  ...
  or raise `ValueError`. This makes behavior easy to localize: an endpoint's logic lives
  entirely in the service it calls.
...
- **Association tables with payload.** `playlist_entries` carries `position`, `added_by`,
  and `added_at`, so playlist ordering is a property of the join row, not the song itself.
